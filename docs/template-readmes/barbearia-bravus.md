# Barbearia Bravus

Atmosfera cinematográfica com scroll coreografado.

✨ **Free** — disponível para todos os membros do [WX Blueprints](https://wxblueprints.com).

---

## 🎯 Demo ao vivo

**Deploy:** https://barbearia-bravus-ofc.vercel.app/

Este é um dos 7 templates oficiais do **WX Blueprints** — marketplace de blueprints prontos para IA coding (Lovable, Bolt, Cursor, Claude).

## 📦 Stack

- HTML
- CSS
- JS
- GSAP
- Lenis

## 🧩 Seções incluídas

1. Hero
2. Apple Sequence
3. About
4. Services
5. Experts
6. Contact

## 🎨 Identidade visual

| Papel | Cor |
|-------|-----|
| Background | `#0f0f0f` |
| Primary | `#d61c1c` |
| Accent | `#f5f5f5` |
| Muted | `#b3b3b3` |

**Tipografia**
- Display: `Chakra Petch`
- Body: `Montserrat`

## 🚀 Como rodar localmente

```bash
git clone https://github.com/Wesleysc94/Barbearia-Bravus-Blueprint-WX.git
cd Barbearia-Bravus-Blueprint-WX
Abra `index.html` no navegador (ou use `npx serve .`)
```

Abra [http://localhost:3000](http://localhost:3000) (ou a porta que o dev server informar).

## 🎨 Como customizar para outro cliente

O blueprint técnico completo (com prompt universal, design tokens, spec de animações e checklist de QA) está disponível em:

👉 **https://wxblueprints.com/blueprints/barbearia-bravus**

Resumo rápido do que alterar:

1. **Cores** — edite as CSS vars em `src/styles/globals.css` (ou equivalente)
2. **Fontes** — Chakra Petch para títulos, Montserrat para corpo
3. **Textos** — cada seção tem dados hardcoded ou em `src/data/*.ts`
4. **Imagens** — substitua em `public/images/` mantendo as proporções
5. **Seções** — manter a ordem e a anatomia, alterar apenas conteúdo



## 🤖 Usando com IA coding

Copie o prompt universal completo em [WX Blueprints — Barbearia Bravus](https://wxblueprints.com/blueprints/barbearia-bravus) e cole em:

- **Lovable** — cria a estrutura em minutos com design fiel
- **Bolt.new** — ideal para MVP rápido
- **Cursor / Claude Code** — melhor para clonagem 1:1 com alta fidelidade

## 📜 Licença

Uso **individual e comercial** permitido para o assinante (1 projeto por cliente). **Proibido redistribuir**, revender ou publicar o código em repositórios públicos. Detalhes em https://wxblueprints.com/licenca.

## 🔗 Links

- **Blueprint técnico completo:** https://wxblueprints.com/blueprints/barbearia-bravus
- **Demo ao vivo:** https://barbearia-bravus-ofc.vercel.app/
- **WX Blueprints (marketplace):** https://wxblueprints.com
- **Suporte:** wxdigitalstudio@gmail.com

---

Criado por [WX Digital Studio](https://wxblueprints.com) · Qualidade ★★★★★
